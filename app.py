...
st.sidebar.header("☁️ Cloud Settings")
COLAB_URL = st.sidebar.text_input("Colab API URL", placeholder="https://adunc-nonluminous-alberto.ngrok-free.dev/")
...
# --- BLOCK 3: GENERATE SPEECH (CLOUD + SMART SPEED) ---
with col3:
    st.header("3. Generate Speech")
    st.info(f"Output: {audio_filename}")
    
    # Check if COLAB_URL exists
    if 'COLAB_URL' not in globals() or not COLAB_URL:
        COLAB_URL = st.text_input("Enter Ngrok URL (from Colab):", placeholder="https://xxxx.ngrok-free.app")

    if st.button("▶ Generate Speech (Cloud)"):
        if not COLAB_URL:
            st.error("Please enter the Colab/Ngrok URL!")
        else:
            with st.spinner(f"Sending text to Cloud (Target: {target_duration}s)..."):
                try:
                    import requests
                    
                    # Send text AND target_duration to Colab
                    response = requests.post(
                        f"{COLAB_URL}/generate_audio", 
                        data={
                            "text": script_content, 
                            "target_duration": float(target_duration) # This comes from Block 2
                        } 
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        st.success("✅ Audio received! Exact duration matched.")
...
# --- BLOCK 4: SUBTITLES (CLOUD VERSION) ---
with col4:
    st.header("4. Subtitles")
    
    if st.button("Generate SRT (Cloud)"):
        # Check if URL is set (from Block 3 or Sidebar)
        if 'COLAB_URL' not in globals() or not COLAB_URL:
            st.error("No Colab URL found! Please enter it in the Speech section.")
        elif not os.path.exists(full_audio_path):
            st.error("Audio not found! Generate speech first.")
        else:
            with st.spinner("Uploading audio to Colab for transcription..."):
                try:
                    import requests
                    
                    # Upload the generated audio to Colab
                    ...
                    if response.status_code == 200:
                        ...
                        # Convert the JSON response into valid SRT format locally
                        with open(final_srt_path, "w", encoding="utf-8") as srt_file:
                            for i, segment in enumerate(data["segments"]):
                                # Uses your existing format_timestamp utility function
                                start = format_timestamp(segment["start"])
                                end = format_timestamp(segment["end"])
                                text = segment["text"].strip().replace(".", "")
                        ...
                        st.success(f"SRT Generated via Cloud!")
                        with open(final_srt_path) as f:
                            st.download_button("Download SRT", f.read(), srt_filename)
                    else:
                        st.error(f"Cloud Transcription Failed: {response.text}")
